...
usage() {
    ...
    echo "  --data[folder=PATH]        Bind mount for rose data folder (by default will create the 'rose' volume)."
    ...
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --webui[port=3000] --data[folder=./rose-data]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --webui[port=3000] --data[folder=./rose-data] --build"
    echo ""
...
if [[ $kill_compose == true ]]; then
    ...
    if [[ $enable_gpu == true ]]; then
        ...
        if [[ -n $gpu_count ]]; then
            ...
            export ROSE_GPU_DRIVER=$(get_gpu_driver)
            export ROSE_GPU_COUNT=$gpu_count # Set ROSE_GPU_COUNT environment variable
        fi
    ...
    if [[ $enable_api == true ]]; then
        ...
        if [[ -n $api_port ]]; then
            export ROSE_WEBAPI_PORT=$api_port # Set ROSE_WEBAPI_PORT environment variable
        fi
    fi
    if [[ -n $data_dir ]]; then
        DEFAULT_COMPOSE_COMMAND+=" -f docker-compose.data.yaml"
        export ROSE_DATA_DIR=$data_dir # Set ROSE_DATA_DIR environment variable
    fi
...
echo -e "   ${GREEN}${BOLD}GPU Driver:${NC} ${ROSE_GPU_DRIVER:-Not Enabled}"
echo -e "   ${GREEN}${BOLD}GPU Count:${NC} ${ROSE_GPU_COUNT:-Not Enabled}"
echo -e "   ${GREEN}${BOLD}WebAPI Port:${NC} ${ROSE_WEBAPI_PORT:-Not Enabled}"
echo -e "   ${GREEN}${BOLD}Data Folder:${NC} ${data_dir:-Using rose volume}"
